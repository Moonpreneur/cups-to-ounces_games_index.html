    ...
    selectedQuestions.forEach((q, index) => {
        const questionHTML = `
            <div style="margin: 20px; padding: 10px; border-radius: 10px;">
                <span class="first" span id="first${index}" ondrop="drop(event)" ondragover="allowDrop(event)">Drag Measurement here</span>
                + <span class="second" span id="second${index}" ondrop="drop(event)" ondragover="allowDrop(event)">Drag Measurement here</span>
                + <span class="third" span id="third${index}" ondrop="drop(event)" ondragover="allowDrop(event)">Drag Measurement here</span> =
                <span id="targetOunces${index}" style="font-weight: bold;">${q.targetOunces}</span>
...
function generateValues() {
    ...
    values.forEach(value => {
        const valueHTML = `
            <td><div draggable="true" ondragstart="drag(event)" id="${value} cups" style="background-color: #ffcccc; border: 2px solid #333; border-radius: 5px; padding: 5px; cursor: move;">${value} cups</div></td>
        `;
...
function checkAnswer(index) {
    const first = parseInt(document.getElementById(`first${index}`).innerText);
    const second = parseInt(document.getElementById(`second${index}`).innerText);
    const third = parseInt(document.getElementById(`third${index}`).innerText);

    const targetOunces = selectedQuestions[index].targetOunces;

    if (!isNaN(first) && !isNaN(second) && !isNaN(third)) {
        const calculatedOunces = 8 * (first + second + third) + ' ounces';
        if (calculatedOunces === targetOunces) {
            if (!document.getElementById(`result${index}`).innerText) {
                document.getElementById('result').innerText = `Correct! The total number of ounces is ${calculatedOunces}.☺️`;
